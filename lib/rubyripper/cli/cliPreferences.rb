 ...
 maximum" : @prefs.maxTries]
  ...
  def loopSubMenuRipping
    case choice = showSubMenuRipping()
      ...
      when 7 then @prefs.reqMatchesAll = @int.get(_("Match all chunks"), 2)
      when 8 then @prefs.reqMatchesErrors = @int.get(_("Match erronous chunks"), 3)
      when 9 then @prefs.maxTries = @int.get(_("Maximum trials"), 5)
  ...
  def setMetadataProvider
    choices = [['none', _("Don't fetch metadata from the internet")],
      ['gnudb', _('Use the gnudb protocol as primary resource')],
